...
function _generateRandomParagraph() {
    ...
    $numWords = Get-Random -Maximum 30 -Minimum 10
...
# The number of incorrect characters typed in total, used to track accuracy.
...
Write-Host "WPM: $wpm"
Write-Host "Accuracy: $([Math]::Round(($text.Length/($text.Length + $totalIncorrectCharacters)) * 100.0))%"
